...
export default function AdminUsersManager() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      setFeedback(variables.isActive ? t('activateSuccess') : t('deactivateSuccess'));
  ...
  async function handleDelete(item: AdminUser) {
    const confirmed = window.confirm(
      t('deleteConfirm', {name: item.fullName})
    );
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label={t('stats.active')} value={allItems.filter((item) => item.isActive).length} tone="emerald" />
        <StatCard label={t('stats.superAdmins')} value={allItems.filter((item) => item.role === 'SUPER_ADMIN').length} tone="blue" />
        <StatCard label={t('stats.admins')} value={allItems.filter((item) => item.role === 'ADMIN').length} />
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          <p className="text-sm font-semibold text-slate-900">{t('studioTitle')}</p>
      ...
      {adminsQuery.isPending ? (
        ...
        <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">{t('emptyDescription')}</p>
        </div>
      ) : (
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                      ...
                      {item.role === 'SUPER_ADMIN' ? t('superAdminRole') : t('adminRole')}
                    </span>
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                      item.isActive
                  ...
                  <div className="space-y-1 text-sm text-slate-600">
                    <p>{item.email}</p>
                    <p>{t('lastLogin')}: {formatDate(item.lastLoginAt)}</p>
                    <p>{t('createdAt')}: {formatDate(item.createdAt)}</p>
                  </div>
                </div>

                <div className="flex flex-wrap gap-2">
                  <Link href={`/admin/users/${item.id}/edit`}>
                    <Button type="button" variant="outline">{t('edit')}</Button>
                  </Link>
                  <Button type="button" variant="outline" onClick={() => setResetState({adminId: item.id, name: item.fullName})}>
                    {t('resetPassword')}
                  </Button>
                  <Button type="button" variant="outline" onClick={() => handleToggleStatus(item)} isLoading={statusMutation.isPending}>
                    {item.isActive ? t('deactivate') : t('activate')}
                  </Button>
                  <Button type="button" variant="danger" onClick={() => handleDelete(item)} isLoading={deleteMutation.isPending}>
                    {t('delete')}
                  </Button>
                </div>
              </div>
      ...
      {resetState ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 p-4" onClick={() => setResetState(null)}>
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <h3 className="text-xl font-bold text-slate-900">{t('resetPasswordTitle', {name: resetState.name})}</h3>
            <p className="mt-2 text-sm text-slate-600">{t('resetPasswordSubtitle')}</p>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setResetState(null)}>
                {common('cancel')}
              </Button>
              <Button type="button" onClick={handleResetPassword} isLoading={resetMutation.isPending}>
                {t('resetPassword')}
              </Button>
            </div>
          </div>
