...
type TypeFilter = 'ALL' | PortfolioProjectMediaType;
...
export default function PortfolioProjectMediaManager({projectId}: Props) {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<PortfolioProjectMediaResponse | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: deletePortfolioProjectMedia,
    onSuccess: async () => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async (item: PortfolioProjectMediaResponse) => {
      ...
      return updatePortfolioProjectMedia(item.id, payload);
    },
  ...
  function handleDelete(item: PortfolioProjectMediaResponse) {
  ...
  async function handleToggleStatus(item: PortfolioProjectMediaResponse) {
    setFeedback('');
  ...
  return (
    <div className="space-y-6">
      ...
      {projectQuery.isPending || mediaQuery.isPending ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-600">{common('loading')}</p>
        </div>
      ) : projectQuery.isError || mediaQuery.isError ? (
        <div className="rounded-3xl border border-red-200 bg-red-50 p-6 shadow-sm">
          <p className="text-sm text-red-700">
            {getErrorMessage(
              toAppError(projectQuery.error || mediaQuery.error),
              (key) => errorT(key)
            )}
          </p>
        </div>
